...
object Day2 {
    ...
    private data class Game(val id: Int, val rounds: List<Map<Cube, Int>>)
    ...
    private fun part1(games: List<Game>) {
        val bag = mapOf(Cube.Red to 12, Cube.Green to 13, Cube.Blue to 14)
        val sumOfIds = possible(bag, games)
        println("Sum of possible game IDs: $sumOfIds")
    }

    private fun part2(games: List<Game>) {
        val power = games.sumOf { game ->
            val mostRed = game.rounds.maxBy { it[Cube.Red].orZero() }[Cube.Red]!!
            val mostGreen = game.rounds.maxBy { it[Cube.Green].orZero() }[Cube.Green]!!
            val mostBlue = game.rounds.maxBy { it[Cube.Blue].orZero() }[Cube.Blue]!!
            mostRed * mostGreen * mostBlue
        }
        println("Power total: $power")
    }

    private fun parseGames(lines: List<String>): List<Game> {
        return lines.mapIndexed { id, line ->
            val rawRounds = line.split(": ")[1].split("; ")
            val rounds = rawRounds.map { round ->
                round.split(", ").associate { dice -> round(dice) }
            }
            Game(id + 1, rounds)
        }
    }

    private fun round(string: String): Pair<Cube, Int> {
        val digit = Regex("\\d+")
        val color = Regex("red|green|blue", RegexOption.IGNORE_CASE)
        val number = digit.find(string)!!.value.toInt()
        return when (color.find(string)?.value) {
            "red" -> Cube.Red to number
            "green" -> Cube.Green to number
            "blue" -> Cube.Blue to number
            else -> throw IllegalArgumentException()
        }
    }

    private fun possible(bag: Map<Cube, Int>, games: List<Game>): Int {
        return games.sumOf { game ->
            val possible = game.rounds.all { round ->
                val red = round[Cube.Red].orZero() <= bag[Cube.Red].orZero()
                val green = round[Cube.Green].orZero() <= bag[Cube.Green].orZero()
                val blue = round[Cube.Blue].orZero() <= bag[Cube.Blue].orZero()
                red && green && blue
            }
