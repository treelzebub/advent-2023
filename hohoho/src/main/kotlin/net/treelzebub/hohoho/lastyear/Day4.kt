...
object Day4 {
    ...
    private fun part1() {
        val count = parse().count { fullyContains(it) }
        println("Input contains $count full overlaps")
    }
    ...
    private fun fullyContains(pair: Pair<IntRange, IntRange>): Boolean {
        return fullyContains(pair.first, pair.second) || fullyContains(pair.second, pair.first)
    }
    ...
    private fun part2() {
        val count = parse().count { overlaps(it) }
        println("There are $count at least partial overlaps.")
    }

    private fun overlaps(pair: Pair<IntRange, IntRange>): Boolean {
        return overlaps(pair.first, pair.second) || fullyContains(pair.second, pair.first)
    }
